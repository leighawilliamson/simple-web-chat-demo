...
async function predictions (params){
    ...
    for (i in response.body.data.predictionsData){
        // add route name
        routes = routes + "Route " + response.body.data.predictionsData[i].routeName;

        // add all destinations for the route
        for (j in response.body.data.predictionsData[i].destinations) {
            routes = routes + " <break time=\"200ms\"></break>heading to " + response.body.data.predictionsData[i].destinations[j].headsign;
            routes = routes + " will stop here at ";
            ...
            for (k in response.body.data.predictionsData[i].destinations[j].predictions){
                ...
                if (k == lasttime) {routes = routes + " <break time=\"300ms\"></break> ";} // comma after last time for destination
                else {routes = routes + " and ";} // if not last time for destination, insert the word and, and add next time
            }

        }
        // add a pause in between each route
        routes = routes + "<break time=\"300ms\"></break>";
    }

    // format the routes information returned by the API call
    routes2 = "Next buses arriving at bus stop " + params.stopcode + ":\n";
    for (i in response.body.data.predictionsData){
        // add route name
        routes2 = routes2 + "Route " + response.body.data.predictionsData[i].routeName + "\n";

        // add all destinations for the route
        for (j in response.body.data.predictionsData[i].destinations) {
            routes2 = routes2 + "To: " + response.body.data.predictionsData[i].destinations[j].headsign + "\n";

            // add times (one or more) for each destination
            routes2 = routes2 + "Arriving at: ";
            var lasttime = Object.keys(response.body.data.predictionsData[i].destinations[j].predictions).length - 1;
            for (k in response.body.data.predictionsData[i].destinations[j].predictions){
                utctime = response.body.data.predictionsData[i].destinations[j].predictions[k].time;
                routes2 = routes2 + format_date(utctime);
                if (k == lasttime) {routes2 = routes2 + "\n";} // newline after last time for destination
                else {routes2 = routes2 + " & ";} // if not last time for destination, put ampersand and add next time
            }
        }
...
async function predictions2 (params){
    // easy debug printout
    // console.log("entering predictions function");
    var routes = "";
    var utctime = "";
